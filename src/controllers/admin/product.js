    ...
    uploadProduct(req, res, async function (err) {
        ...
        try {
            ...
            if (!size || !quantity || !price) {
                return res.status(HTTP_STATUS.BAD_REQUEST).json({ size: 'Fill size fields' });
            }
...
const listProducts = async (req, res) => {
    ...
    try {
        ...
        res.render("backend/admin-dashboard.ejs", {
            message: '', admin: req.session.admin.email, partial: "partials/product-list", products, totalPages,
...
const deleteSize = async (req, res) => {
    try {
        ...
        res.status(HTTP_STATUS.OK).json({ message: 'Size deleted successfully' });
    } catch (err) {
        console.log(err);
        res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).render("frontend/404");
    }
...
const deleteProductImage = async (req, res) => {
    try {
        ...
        if (fs.existsSync(filePath)) {
            ...
            res.status(HTTP_STATUS.NOT_FOUND).json({ success: false, message: 'File not found on the server' });
        }
...
const blockProduct = async (req, res) => {
    try {
        ...
        await product.save();
        res.status(HTTP_STATUS.NOT_FOUND).json({ status: true });
    } catch (err) {
        console.log(err);
        res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).render("frontend/404");
    }
...
const deleteproduct = async (req, res) => {
    try {
        ...
        // Respond to client
        res.status(HTTP_STATUS.OK).json({ success: true, message: 'Product and images deleted successfully' });
    } catch (err) {
        console.log(err);
        res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).render("frontend/404");
    }
