...
const verifyPayment = async (req, res) => {
  ...
  if (hash === razorpay_signature) {
    ...
    try {
      ...
      let totalProductPrice = cartItems.reduce((total, item) => {
    ...
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ status: false, message: "Payment Verification Failed" });
  }
};
...
const paymentFailed = async (req, res) => {
  ...
  lock.acquire(razorpay_order_id, async (done) => {
    ...
    try {
      ...
      if (!cart) {
        return res.status(HTTP_STATUS.NOT_FOUND).json({ status: false, error: 'Cart not found' });
      }
      ...
      const newOrder = new Order({
        ...
        items: cartItems,
        paymentMethod,
        totalPrice: parseFloat(totalProductPrice - cart.walletBalance - cart.couponBalance).toFixed(2),
        appliedWallet: cart.walletBalance,
        appliedCoupon: cart.couponBalance,
      ...
      res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ status: false, error: 'Order creation failed after payment failure' });
    }
