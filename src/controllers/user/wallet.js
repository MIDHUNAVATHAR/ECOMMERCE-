...
const walletAddCart = async (req, res) => {
    try {
        ...
        if (user.walletBalance == 0) {
            return res.status(HTTP_STATUS.OK).json({ success: false, message: "No wallet Balance to Apply" })
        }
        ...
        if (cartTotal - cart.couponBalance < user.walletBalance) {
            return res.status(HTTP_STATUS.OK).json({ success: false, message: `Cart should have minimum ${user.walletBalance} for order` })
        }
...
const walletRemoveCart = async (req, res) => {
    try {
        ...
        console.log(err.message);
        res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ user: false });
    }
}
