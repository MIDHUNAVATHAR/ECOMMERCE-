...
const getCart = async (req, res) => {
  try {
    ...
    if (cart) {
      ...
      walletDiscount = parseFloat(walletDiscount).toFixed(2);

      cart.walletBalance = walletDiscount;
      ...
      return res.render("frontend/cart.ejs", {
        logo,
        user,
        userId,
        cart,
  ...
  try {
    ...
    if (imagePath) {
      ...
      if (fs.existsSync(imageFilePath)) {
        ...
        return res.status(HTTP_STATUS.NOT_FOUND).json({ message: 'Image not found', success: false });
      }
    }
    ...
    await cart.save()
      .then(() => {
        return res.status(HTTP_STATUS.OK).json({ message: "Product added to cart", success: true });
      })
  ...
  try {
    ...
    if (cart.items[0].quantity > 1) {
      ...
      if (cart.items[0].quantity <= product.sizes[0].quantity + 1) {
        ...
        return res.status(HTTP_STATUS.BAD_REQUEST).json({ message: `Product unavailable .Only ${product.sizes[0].quantity} are left! `, success: false });
      }
    } else {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ message: "Quantity must be minimum One", success: false });
    }
...
const removeItem = async (req, res) => {
  try {
    ...
    cart.save().then(() => {
      return res.status(HTTP_STATUS.OK).json({ message: "Item remove Successfully" });
