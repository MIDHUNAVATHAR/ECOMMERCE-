...
const  userAdressMng  =  async  ( req , res )  =>{
   try{
      const logo = await Logo.findOne().sort({ updatedAt: -1 }); 
      const genderCategory = await GenderCategory.find({ softDelete : false }) ; 
      const user = await User.findById( req.session.userId  ||  req.user._id  ); // assuming user is authenticated and stored in session 
      ...
      res.render("frontend/userAdressManage.ejs" , { logo , genderCategory , user , addresses , cartTotal  } ) ;
...
const deleteAddress = async ( req,res ) =>{
 ...
 const  changePassword  =  async  ( req , res ) => { 
   try{
      ...
      res.render("frontend/changePassword.ejs" , { logo  , genderCategory , user , cartTotal  } ) ; 
