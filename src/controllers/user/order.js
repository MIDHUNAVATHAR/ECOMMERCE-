...
const placeorder = async (req, res) => {
  try {
    ...
    if (cart && cart.items.length == 0) {
      return res.status(400).json({ status: false, message: "No items in the cart" });
    }
...
const cancelOrder = async (req, res) => {
  try {
    ...
    res.status(HTTP_STATUS.OK).json({ status: true, wallet });
  } catch (err) {
    console.error(err);
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).render("frontend/404");
  }
...
const submitReview = async (req, res) => {
  try {
    ...
    res.status(HTTP_STATUS.OK).json({ success: true, message: 'Review added successfully!' });
  } catch (err) {
    console.error(err);
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).render("frontend/404");
  }
