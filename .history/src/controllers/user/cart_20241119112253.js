...
const getCart = async ( req,res ) => { 
   ...
   const totalPrice = parseFloat( to )
    ...
    await cart.save(); 
   console.log(totalP)
    return res.render("frontend/cart.ejs" , { logo , user , userId , cart , totalItems , totalAmount , couponDiscount, totalPrice : totalP  , genderCategory ,cartTotal }) ; 
  }
  else{
...
const increQuantity = async (req, res) => {
  try {
    ...
    const totalAmount = totalPrice - (updatedCart.walletBalance + updatedCart.couponBalance);
        ...
        if(cart.items[0].quantity > 1){
          ...
          if(cart.items[0].quantity <= product.sizes[0].quantity+1 ){
           ...
           const updatedCart =  await Cart.findOneAndUpdate(
            ...
            return res.status(200).json({ message : "one quantity removed" ,quantity, totalPrice, totalItems , status , totalAmount :  parseFloat(totalAmount.toFixed(2)) , success : true }); 
