...
const  walletAddCart  =  async  ( req , res ) =>{
    try{
        ...
        if(cartTotal >= walletAmount){
            ...
            cart.walletBalance = parseFloat(cartTotal).toFixed(2);
            user.walletBalance = parseFloat(walletAmount - cartTotal).toFixed(2); 
            console.log("fff"parseFloat(cartTotal).toFixed(2))
            console.log("vvv:",parseFloat(walletAmount - cartTotal).toFixed(2))
        }
        await  cart.save();
        await  user.save();
