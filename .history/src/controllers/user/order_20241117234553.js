...
const  placeorder  =  async  ( req , res )  =>{
    try{
        ...
        .filter( async item => item.status === "Available") // filter items by status
        .map( item =>{
    ...
    const imageUrl = product.images[0];  // You can customize this to pick the image you want
...
const  myOrders  =  async  ( req , res ) => {
  try{
    ...
    res.render("frontend/orders" , { orders  , logo , genderCategory ,user , years , orderStatus , orderTime : "" , cartTotal } );  
  }catch(err){
    console.error(err);
    res.status(500).render("frontend/404");  
  }
...
const viewOrder  =  async  ( req , res ) =>{
  try{
    ...
    res.render("frontend/viewOrder.ejs" , { logo , genderCategory , user , order , orderDate , cartTotal }) ; 
  }catch(err){
    console.error(err);
    res.status(500).render("frontend/404");  
  }
